...
public class CourseDataHandler {
    ...
    public List<Discussion> setForumDiscussions(int forumId, List<Discussion> discussions) {
        ...
        List<Discussion> newPartsInDiscussions = new ArrayList<>();

        Realm realm = Realm.getInstance(MyApplication.getRealmConfiguration());
        //check if discussion initially had no data
        if (realm.where(Discussion.class).equalTo("id", forumId).findFirst() == null) {
            for (Discussion discussions : Discussion) {
                discussions.setForumId(forumId);
            }
            realm.beginTransaction();
            realm.copyToRealmOrUpdate(Discussion);
            realm.commitTransaction();
            newPartsInDiscussions = discussionList; //returns the whole sectionList as the whole course is new

        } else {        //not a new course, compare parts for new data
            for (Discussion discussions : discussionList) {
                if (realm.where(Discussion.class).equalTo("id", forumId).findFirst() == null) {
                    //whole section is new
                    discussions.setForumId(forumId);
                    newPartsInDiscussions.add(discussions);
                } else {
                    Discussion realmDiscussion =
                            realm.where(Discussion.class).equalTo("id", forumId).findFirst();
                    Discussion newPartInDiscussions = getNewParts(discussions, realmDiscussion, realm);
                    if (newPartInDiscussions != null) {
                        newPartsInDiscussions.add(newPartInDiscussions);
                    }
                    discussions.setForumId(forumId);
                    // newPartsInSections.add(trimmedSection);
                }
            ...
            realm.beginTransaction();
            realm.where(Discussion.class).equalTo("id", forumId).findAll().deleteAllFromRealm();
            realm.copyToRealmOrUpdate(discussionList);
            realm.commitTransaction();
        }
        realm.close();
        return newPartsInDiscussions;
